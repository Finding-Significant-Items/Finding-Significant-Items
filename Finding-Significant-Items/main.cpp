...
using namespace std;
map <string ,int> B,C,End,Value,Value2,mp,MP;
int co_f,co_p;
...
int main()
{
    ...
    for (map<string,int> :: iterator sit = mp.begin(); sit!=mp.end(); sit++) End[sit->first]++;  // excecute the last period
    CNT=0; Value.clear(); Value2.clear();
    for (map<string,int>::iterator sit = End.begin(); sit!=End.end(); sit++)  
    {
        t[++CNT].x=sit->first;
        t[CNT].y=sit->second*co_p+MP[sit->first]*co_f;
    }
    cout<<CNT<<endl;
    sort(t+1,t+CNT+1,CMP);
    for (int i=1; i<=K; i++) Value[t[i].x]=t[i].y; 
    ...
    {
        for (sh_M=1; 432*K+sh_M*8*16<=MEM*1024*8; sh_M++);
        SH=new Sketchpheap(sh_M,K,MEM*1024*8/(int(log(pd)/log(2))),co_p,co_f);
        for (int i=1; i<=m; i++)
        {
            SH->Insert(i,s[i],Time[i]/WIN+1);
        }
