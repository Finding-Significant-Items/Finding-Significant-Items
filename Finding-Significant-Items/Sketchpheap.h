...
private:
    ...
    int tot[Count][MAX_MEM],head[N],next[Total],m,k,WZ,Q,p,cnt,o,MIN,M2,K,NUM,CBF_M,CBF_C[50000005],TOT[Count][MAX_MEM];
    struct Node {int wz; string ID;} ID_index[Total];

public:
    Sketchpheap(int NUM,int K,int CBF_M,int co_p,int co_f):NUM(NUM),K(K),CBF_M(CBF_M),co_p(co_p),co_f(co_f)
    {
        M2=1000000;
    ...
    void Insert(int x,string A,int t)
    {
        int MIN2=10000000;
        ...
        if (p==-1)
        {
            ...
            if (cnt>K)
            {
                Change(1,cnt,x); Delete(x,(bobhash[Count]->run(Heap[cnt].ID.c_str(),Heap[cnt].ID.size()))%M2,Heap[cnt].ID);
                int now=1; cnt--;
